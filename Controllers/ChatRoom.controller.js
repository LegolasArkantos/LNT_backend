...
const getAllChatRooms = async (req, res) => {
    ...
    try {
        ...
        const result = await Teacher.findById(req.user.profileID).select('chatRooms').populate({
            path: 'chatRooms',
            populate: [
                {
                    path: 'participants',
        ...
        res.status(200).send(result)
            ...
            const result = await Student.findById(req.user.profileID).select('chatRooms').populate({
                path: 'chatRooms',
                populate: [
                    {
                        path: 'participants',
...
const createChatRoom = async (req,res) => {
    ...
    try{
        const result = await ChatRoom.findOne({participants: {$all: req.body.participantID}});
        if (result == null) {
            ...
            for (let i = 0; i < req.body.participants.length; i++){
                const participantRef = chat.schema.path('participants.' + i).options.ref;

                if (participantRef == "Teacher"){
                    const teacher = await Teacher.findById(req.body.participants[i]);
                    if (teacher) {
                    ...
                    const student = await Student.findById(req.body.participants[i]);
                    if (student) {
...
module.exports = { getAllChatRooms, deleteChatRoom, createChatRoom, deleteMessage };